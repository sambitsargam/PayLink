...
const Transaction = (props) => {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 5001) {
      window.alert("Change the network to Mantle Network");
      throw new Error("Change network to Mantle Network");
    }
    if (needSigner) {
  ...
  const handleClick = () => {
    window.open(
      `https://explorer.sepolia.mantle.xyz/address/${props.contractAddress}`
    );
  };
        ...
        <>
          ...
          <Box textAlign="center" justifyContent="center" justify="center">
            <>
              ...
              <Grid sx={{ mb: 3 }}>
                <Link
                  href={`https://explorer.sepolia.mantle.xyz/address/${props.contractAddress}`}
                  underline="hover"
                  target="_blank"
                >
                  View Contract On Mantle Explorer
                </Link>
              </Grid>
            </>
          </Box>
